...
class DateResolverDialog(CancelAndHelpDialog):
    ...
    async def initial_step(
        ...
        print('\nDateResolverDialog generic track')
...
class DateResolverDialog_start(CancelAndHelpDialog):
    def __init__(self, dialog_id: str = None):
        super(DateResolverDialog_start, self).__init__(
            dialog_id or DateResolverDialog_start.__name__
        )

        self.add_dialog(
            DateTimePrompt(
                DateTimePrompt.__name__, DateResolverDialog_start.datetime_prompt_validator
            )
        )
        self.add_dialog(
            WaterfallDialog(
    ...
    async def initial_step(
        ...
        print('\nDateResolverDialog start track')

        prompt_msg_text = "On what date would you like to start your travel ?"
        prompt_msg = MessageFactory.text(
            prompt_msg_text, prompt_msg_text, InputHints.expecting_input
        )
...
class DateResolverDialog_end(CancelAndHelpDialog):
    def __init__(self, dialog_id: str = None):
        super(DateResolverDialog_end, self).__init__(
            dialog_id or DateResolverDialog_end.__name__
        )

        self.add_dialog(
            DateTimePrompt(
                DateTimePrompt.__name__, DateResolverDialog_end.datetime_prompt_validator
            )
        )
        self.add_dialog(
            WaterfallDialog(
    ...
    async def initial_step(
        ...
        print('\nDateResolverDialog end track')

        prompt_msg_text = "On what date would you like to end your travel ?"
        prompt_msg = MessageFactory.text(
            prompt_msg_text, prompt_msg_text, InputHints.expecting_input
        )
